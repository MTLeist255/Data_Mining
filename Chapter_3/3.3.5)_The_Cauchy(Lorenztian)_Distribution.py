...
# Figure 3.11.
#
# This shows an example of a Cauchy distribution with various parameters. We’ll generate the distribution using:
#
# dist = scipy.stats.cauchy(...)
# Where … should be filled in with the desired distribution parameters Once we have defined the
# distribution parameters in this way, these distribution objects have many useful methods; for
# example:
#
...
plt.title('Figure 3.11) Cauchy Distribution')
...
# Figure 3.12.
#
# The bottom panel shows a sample of N points drawn from a Cauchy distribution with \mu = 0 and
# \gamma=2. The top panel shows the sample median, sample mean, and two robust estimates of the
# location parameter (see text) as a function of the sample size (only points to the left from a
# given sample size are used). Note that the sample mean is not a good estimator of the distribution’s
# location parameter. Though the mean appears to converge as N increases, this is deceiving: because
# of the large tails in the Cauchy distribution, there is always a high likelihood of a far-flung
# point affecting the sample mean. This behavior is markedly different from a Gaussian distribution
# where the probability of such “outliers” is much smaller.
...
ax.set_title('Figure 3.12) Median and Mean for Cauchy Distribution')
